...
Read alongside the pass-1 retrospective (`docs/history/pass-1-retrospective/2026-05-06-lessons-learned.md`, `2026-05-06-pass-2.md`, `2026-05-06-path-forward.md`) and the working-memory entries the pass-1 implementation accumulated. Pass 1 is the reference implementation; main retains it. Pass 2 is greenfield on a long-lived branch and merges back only when behavior parity is reached.
...
Lifted from `docs/history/pass-1-retrospective/2026-05-06-pass-2.md` § "Decisions worth keeping verbatim":
...
These six documents at `docs/` root are the design backbone. They get drafted before any new generation code.
...
This document distills the conversation across the pass-2 design session. The retrospective trio — `docs/history/pass-1-retrospective/2026-05-06-{lessons-learned,pass-2,path-forward}.md` — and the working-memory entries on this project are the inputs. Where the post-completion pass-2 doc and the mid-pass strategic reflection diverged (decomposition granularity, harness-first sequencing, dependency-graph-first design, API discipline), pass 2 takes the mid-pass position deliberately.
