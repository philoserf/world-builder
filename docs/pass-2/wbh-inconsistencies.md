...
- Each decision has at least one test that asserts the chosen value and documents the alternative as a comment (e.g., `// Book worked example shows "+3" giving 9; we follow the formula box (= 6) per docs/pass-2/wbh-inconsistencies.md § Inconsistency 5.`).
