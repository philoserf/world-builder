...
This is a Naur-style theory of `wbh`: the mental model a competent maintainer must hold to change this system without damaging its conceptual integrity. It is meant for an engineer inheriting the project — someone who will be expected to make decisions, not just follow the docs.
...
A single integer seed plus the procedures determine the system fully. Two `wbh -seed 42 -format markdown` invocations on different machines produce byte-identical output.
...
The code calls this cluster's solver `ApplyClimatePasses` (`worlds/stage5.go`). It runs exactly two passes of (compute temperature → compute partial-geology factors → apply the inherent-temperature addition from TSS → rederive atmosphere and hydrographics from the post-TSS temperature). The second pass's result is trusted.
...
The code then reverted to the pass-1 behaviour (two passes, trust the second) and renamed `ConvergeClimate` to `ApplyClimatePasses` in commit `041704d`. The doc-comment on `ApplyClimatePasses` is honest about what is going on. But the framing word "Converge" persists in older design language: `api-surface.md` and `dependency-graph.md` still describe a "fixed-point solver" with caveats; `design-intent.md` calls the function `ApplyClimatePasses` but `dependency-graph.md` § Stage 5 still calls it `ConvergeClimate`. `history/lessons-learned.md` § L13 is the canonical account of what actually happened.
...
## The IISS package and its one asymmetry

`iiss/` (the package, not the in-fiction service) is the boundary between system construction and rendering. The form types — `Class0IForm`, `Class23Form`, `Class4PForm`, `SystemForms`, `FormHeader` — live there. The Markdown renderers (`MarkdownClass0I`, `MarkdownClass23`, `MarkdownClass4P`, `MarkdownSystem`) live there. **`iiss/` does not import `worlds/`.** The boundary type passed across the seam is `iiss.SystemForms`, populated by `worlds.BuildIISSForms` and consumed by the renderers.
...
There is one asymmetry, and you will encounter it the first time you touch Class IV-P. The Class IV-P form is the per-mainworld detail sheet, and its body content is too domain-specific to flatten into renderer logic that doesn't import worlds. The resolution: `iiss.Class4PForm` holds a `RenderBody func(*strings.Builder, FormHeader)` closure. `MarkdownClass4P` writes the form's header chrome and then calls the closure to produce the body. The closure points back to a method on a worlds-side struct (`Class4PPartP.RenderBody`, `Class4PPartPB.RenderBody` in `worlds/iiss_class4p.go`).

So: Class 0/I and Class II/III are fully owned by `iiss/`. Class IV-P body is owned by `worlds/` and crosses the boundary as a closure stored on a form struct. The `PartP` and `PartPB` fields on `Class4PForm` are typed `any` for the same reason — `iiss/` does not know the worlds-side types and cannot import them. This is documented in the form's doc-comment and is the resolution of issue #48 (commit `636fa51`).

The seam works. It is, however, the one place where the renderer-symmetry story told in `api-surface.md` § A.2 is not literally true; treat that doc claim as descriptive of Class 0/I and Class 23 specifically, and read `worlds/iiss_class4p.go` if you need to change anything about the mainworld detail form.
...
- **Bulk sweep.** A one-off `cmd/wbh-bulk` runner (not in the current source tree; lived in a branch during pass 2's clean-every-run project) verifies 10 000 random seeds all produce real, fully-formed systems with no errors outside the Special-Circumstances family. The result that lets v1.0 ship.
...
A new maintainer with the theory will instinctively reach for the dependency graph and the iterator-plus-Has-pointer shape when adding behaviour. A new maintainer without the theory will reach for the chapter in the book, find the corresponding file by name match, and write code that iterates planets only or that reads a pre-climate value when it needs a post-climate one. The anti-patterns catalogue (`anti-patterns.md`) is the explicit list of the latter mistakes; A.1 (moon-path silent-zero) and A.8 (stale pre-fixed-point inputs) are the two that recurred most.
...
- **The climate-solver vocabulary lag.** `design-intent.md` and the function doc-comment honestly describe two-pass sampling; older parts of `dependency-graph.md` and `api-surface.md` still use "convergence" / "fixed-point solver" language. The function was renamed `ConvergeClimate → ApplyClimatePasses` in commit `041704d`, but the older docs and some memory entries (per the project's persistent memory) still use the old name. `history/lessons-learned.md` § L13 is the canonical account; trust it. If you touch the climate code and find the older framing in a doc, update the doc as part of the change.
- **`stage*.go` file numbering.** Pass 2 explicitly rejected book-pagination-as-architecture, but `worlds/stage2.go` through `worlds/stage10.go` still survive as filenames. Inside, the functions are appropriately named (`ApplyDetailFrontEnd`, `ApplyRotationTilt`, etc.). The numbers describe dependency-graph stage indices, not WBH chapter indices, but a reader who jumps straight into `worlds/` and sees `stage2.go` may form the wrong first impression. If you're already in a refactor of those files, consider renaming.
- **Class IV-P closure boundary.** The `RenderBody func(...)` closure on `iiss.Class4PForm` is the one place the otherwise-clean `iiss/` boundary leaks worlds-side rendering logic. The closure works; the doc claim in `api-surface.md` that "all three forms return typed structs from day one" is broadly true but obscures that Class IV-P carries a callback. Treat the design doc's renderer-symmetry claim as 80% true.
- **The bulk-sweep tool's location.** `harness.md` describes a `cmd/wbh-bulk` runner producing 10 000 successes. That binary is not in `main` (`cmd/wbh` is the only command). The sweep was the v1.0 readiness gate, run from a branch. If you need to re-run a bulk sweep against a current build, you will be re-creating the tool from git history (commit `328b37c`) or writing it fresh.
...
- **`MAO` flows into stars from worlds.** `stars.GenerateSystemOpts.MAO` is set from `worlds.MAO` at the top of `GenerateWithRoller`. This is the one place where worlds-side knowledge flows into the stars package via Opts. It's a small thing, but if you ever try to make `stars` truly standalone you will find this thread.
...
- The split between `worlds.MAO` (a package-level constant or function) and `stars.GenerateSystemOpts.MAO` (set from it) is something I read once at the entry point and have not fully traced. If you change the MAO formula, do it at the worlds-side site and verify the stars-side consumer.
...
- Three IISS forms rendered out of one shared aggregate, with the only seam asymmetry living in the Class IV-P body closure.
