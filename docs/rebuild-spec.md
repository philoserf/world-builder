...
> **Landed on `main`.** MAO (the p.39 table, `MAO`, `LacksP39MAORow`, `IsPostStellar`)
> moved into `stars/mao.go`; `GenerateSystemOpts.MAO` callback removed; `GenerateSystem`
> computes the companion-of-giant orbit in-package. `stars/` has no inbound dependency on
> `worlds/`. Behavior byte-identical (regression + gold baselines unchanged). The rest of
> this section is the original spec rationale.
